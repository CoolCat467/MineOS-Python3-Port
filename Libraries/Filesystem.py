...
__all__ = ['address', 'append', 'copy', 'doFile',
           'exists', 'extension', 'get', 'getProxy',
           'hideExtension', 'isDirectory', 'isHidden',
           'lastModified', 'lines', 'list', 'loadfile',
           'makeDirectory', 'mount', 'mounts', 'name',
           'open', 'path', 'read', 'readLines',
           'readTable', 'remove', 'removeSlashes',
           'rename', 'setProxy', 'size', 'unmount',
           'write', 'writeTable']
...
##        print(['bytes', bytes_])
    ...
    def readUnicodeChar(self):
        ...
        byteArray = [string.byte(self.readString(1))]#bytearray(bytes(self.readString(1)))
    ...
    def read(self, format_, bytesSize=1):
        ...
        if isinstance(format_, int):
            ...
            if format_ == 'a':
                ...
                error(f"Bad argument #2 ('a' (whole file), 'l' (line), 'u' (unicode char), 'b' (byte as number) or 'bs' (sequence of n bytes as number) expected, got {type(_format)})")
