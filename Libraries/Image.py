...
####            print(ch)
####            chars.append(ch)
##        chars = [self.data[2+i*4+3] for i in range(len(self.data[2:]) // 4)]
##        chars = [''.join(chars[i*w:(i+1)*w]) for i in range(h)]
##        return '\n'.join(chars)
##    
##    def __repr__(self):
##        return self.__str__()
                ...
                for foreground in groupedPicture[alpha][symbol][background]:
                    ...
                    for y in groupedPicture[alpha][symbol][background][foreground]:
                        file.writeBytes(
                            ...
                            len(groupedPicture[alpha][symbol][background][foreground][y]))
                ...
                for foreground in range(1, foregroundSize):
                    ...
                    for y in range(1, ySize):
                        ...
                        for x in range(1, xSize):
                            set_(picture, file.readBytes(1), currentY, currentBackground, currentForeground, currentAlpha, currentSymbol)
    return None, None
...
def create(width:int, height:int, background=0x0, foreground=0x0, alpha=0x0, symbol=' ', random_=False):
    ...
    for y in range(picture[1]):
        for x in range(picture[0]):
            i = getIndex(x+1, y+1, picture[0])-1
