...
def _realPath(fakePath):
    ...
    return os.path.join(OPENCOMPUTERS, fakePath[1:])
...
class Proxy:
    ...
    def exists(self, path):
        ...
        return os.path.exists(_realPath(path))
...
##        return False, 'File does not exist.'#success int or False, message
        if self.exists(path):
            try:
                size = os.path.getsize(_realPath(path))
            except OSError:
    ...
    def isDirectory(self, path):
        ...
        return os.path.isdir(_realPath(path))
    ...
    def lastModified(self, path):
        ...
        if self.exists(path):
            time = os.path.getmtime(_realPath(path))
    ...
    def list(self, path):
        ...
        if self.exists(path):
            if self.isDirectory(path):
                return os.listdir(_realPath(path)), None
            name = os.path.basename(_realPath(path))
...
##        return False, 'Stream is invalid.'
    ...
    def open(self, path, mode):
        ...
        if mode in ('r', 'rb', 'w', 'wb', 'a', 'ab'):
            ...
            if (readAdd and self.exists(path)) or (not readAdd):
                ...
                name = os.path.split(os.path.dirname(_realPath(path)))[1]
    ...
    def rename(self, fromPath, toPath):
        ...
        if self.exists(fromPath):
            try:
                os.rename(_realPath(fromPath), _realPath(toPath))
            except OSError:
